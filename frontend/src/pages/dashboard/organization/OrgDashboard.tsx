...
export default function OrgDashboard() {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      setShowForm(false)
      setFormSuccess(t('dashboard.post_success'))
      setForm({ title: '', content: '', route: '', province: '', district: '', contact_phone: '', contact_email: '', starts_at: '', expires_at: '' })
      setTimeout(() => setFormSuccess(''), 4000)
    },
    onError: (e: any) => setFormError(e?.response?.data?.detail || t('dashboard.post_error')),
  })
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!form.title.trim() || !form.content.trim()) {
      setFormError(t('dashboard.form_required'))
      return
    }
    const body: any = {
        ...
        <div className="flex items-center justify-between mb-8">
          ...
          {tab === 'relief' && (
            <button className="btn-danger" onClick={() => { setShowForm(v => !v); setFormError(''); setFormSuccess('') }}>
              <Megaphone size={16} /> {showForm ? t('common.close') : t('dashboard.post_notice')}
            </button>
          )}
        </div>
        ...
        <div className="flex gap-1 mb-6 border-b border-gray-800">
          {TABS.map(({ key, label }) => (
            <button
              key={key}
              onClick={() => setTab(key)}
              className={`px-4 py-2 text-sm font-medium transition-colors border-b-2 -mb-px ${tab === key ? 'border-red-500 text-red-500' : 'border-transparent text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white'
                }`}
            >
              {label}
            </button>
          ...
          <div className="space-y-6">
            ...
            {showForm && (
              <div className="card border border-red-900/40">
                <h3 className="text-gray-900 dark:text-white font-semibold mb-4 flex items-center gap-2"><Megaphone size={16} className="text-red-500 dark:text-red-400" />{t('dashboard.post_relief_title')}</h3>
                <form onSubmit={handleSubmit} className="space-y-3">
                  <div>
                    <label className="label">{t('report.title')} *</label>
                    <input className="input" value={form.title} onChange={update('title')} placeholder={t('dashboard.title_placeholder')} required />
                  </div>
                  <div>
                    <label className="label">{t('report.description')} *</label>
                    <textarea className="input min-h-[100px] resize-y" value={form.content} onChange={update('content')} placeholder={t('dashboard.content_placeholder')} required />
                  </div>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    <div>
                      <label className="label">{t('dashboard.route')}</label>
                      <input className="input" value={form.route} onChange={update('route')} placeholder={t('dashboard.route_placeholder')} />
                    </div>
                    <div>
                      <label className="label">{t('profile.province')}</label>
                      <input className="input" value={form.province} onChange={update('province')} placeholder={t('profile.province_placeholder')} />
                    </div>
                    <div>
                      <label className="label">{t('profile.district')}</label>
                      <input className="input" value={form.district} onChange={update('district')} placeholder={t('profile.district_placeholder')} />
                    </div>
                    <div>
                    ...
                    <div>
                      <label className="label">{t('support.starts_at')}</label>
                      <input title={t('support.starts_at')} className="input" value={form.starts_at} onChange={update('starts_at')} type="datetime-local" />
                    </div>
                    <div>
                      <label className="label">{t('support.expires_at')}</label>
                      <input title={t('support.expires_at')} className="input" value={form.expires_at} onChange={update('expires_at')} type="datetime-local" />
                    </div>
                  </div>
                  ...
                  <button type="submit" disabled={createMutation.isPending} className="btn-danger w-full py-3">
                    {createMutation.isPending ? <><Loader2 size={16} className="animate-spin" /> {t('common.posting')}</> : <><Megaphone size={16} /> {t('common.post')}</>}
                  </button>
                </form>
              </div>
            )}
            ...
            {/* My posts list */}
            <div>
              <h3 className="text-gray-900 dark:text-white font-semibold mb-3">{t('dashboard.my_posts')} ({myPosts.length})</h3>
              {reliefLoading ? (
                <p className="text-gray-400 text-sm">{t('common.loading')}</p>
              ) : myPosts.length === 0 ? (
                <div className="card text-center py-8 text-gray-500">{t('dashboard.no_reports')}</div>
              ) : (
                <div className="space-y-3">
                  {myPosts.map((p: any) => (
                    <div key={p.id} className="card flex items-start gap-3">
                      ...
                      <button
                        onClick={() => { if (confirm(t('common.confirm_delete_post'))) deleteMutation.mutate(p.id) }}
